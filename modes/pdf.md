...
**Audit caveat:** With fonts embedded, Chromium emits the webfonts as **subsetted fonts whose name lives in `/FontName` (prefixed like `AAAAAA+SpaceGrotesk…`), not `/BaseFont`**. So a healthy PDF returns *empty* if you grep `/BaseFont` — that is why Rule 3 and the audit command above target `/FontName` instead. Confirm typography with the `qlmanage` thumbnail (Rule 2) or by checking `document.fonts` status (`loaded`, not `error`). A `Times-Roman` entry under `/BaseFont` still flags the CSS-leak bug.
